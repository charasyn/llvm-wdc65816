//===-- WDC65816InstPrinter.cpp - Convert WDC65816 MCInst to asm --------*- C++ -*-===//
//
...
///
/// \file
/// This file contains definitions for an WDC65816 MCInst printer.
///
//===----------------------------------------------------------------------===//
...
void WDC65816InstPrinter::printRegName(raw_ostream &OS, MCRegister Reg) const {
  OS << "%" << getRegisterName(Reg);
}

void WDC65816InstPrinter::printInst(const MCInst *MI, uint64_t Address,
...
void WDC65816InstPrinter::printImmediate(const MCInst *MI, unsigned opNum,
                                     raw_ostream &O) {
...
void WDC65816InstPrinter::printMoveMask(const MCInst *MI, unsigned opNum,
                                    raw_ostream &O) {
  ...
  for (int s = 0; s < 16; s += 8) {
    ...
    for (int i = 0; HalfMask; ++i) {
      if ((HalfMask >> i) & 0b1) {
        HalfMask ^= 0b1 << i;
        Reg = WDC65816II::getMaskedSpillRegister(i + s);
        printRegName(O, Reg);
        ...
        if (j != i) {
          O << '-';
          Reg = WDC65816II::getMaskedSpillRegister(j + s);
          printRegName(O, Reg);
        }
...
void WDC65816InstPrinter::printDisp(const MCInst *MI, unsigned opNum,
                                raw_ostream &O) {
...
void WDC65816InstPrinter::printAbsMem(const MCInst *MI, unsigned opNum,
                                  raw_ostream &O) {
  const MCOperand &MO = MI->getOperand(opNum);
