//===-- WDC65816FrameLowering.cpp - WDC65816 Frame Information ----------*- C++ -*-===//
//
...
WDC65816FrameLowering::WDC65816FrameLowering(const WDC65816Subtarget &STI, Align Alignment)
...
bool WDC65816FrameLowering::hasReservedCallFrame(const MachineFunction &MF) const {
  return !MF.getFrameInfo().hasVarSizedObjects() &&
         !MF.getInfo<WDC65816MachineFunctionInfo>()->getHasPushSequences();
}

bool WDC65816FrameLowering::canSimplifyCallFramePseudos(
...
bool WDC65816FrameLowering::needsFrameIndexResolution(
    const MachineFunction &MF) const {
...
// WDC65816RegisterInfo::eliminateFrameIndex, but not here. Possibly
// (probably?) it should be moved into here.
StackOffset
WDC65816FrameLowering::getFrameIndexReference(const MachineFunction &MF, int FI,
  ...
  const WDC65816MachineFunctionInfo *MMFI = MF.getInfo<WDC65816MachineFunctionInfo>();
...
uint64_t
WDC65816FrameLowering::calculateMaxStackAlign(const MachineFunction &MF) const {
  const MachineFrameInfo &MFI = MF.getFrameInfo();
...
void WDC65816FrameLowering::BuildStackAlignAND(MachineBasicBlock &MBB,
                                           MachineBasicBlock::iterator MBBI,
                                           const DebugLoc &DL, unsigned Reg,
                                           uint64_t MaxAlign) const {
  uint64_t Val = -MaxAlign;
  unsigned AndOp = WDC65816::AND32di;
  unsigned MovOp = WDC65816::MOV32rr;

  // This function is normally used with SP which is Address Register, but AND,
  // or any other logical instructions in WDC65816 do not support ARs so we need
  // to use a temp Data Register to perform the op.
  unsigned Tmp = WDC65816::D0;
...
/// a constant value.
void WDC65816FrameLowering::emitSPUpdate(MachineBasicBlock &MBB,
                                     MachineBasicBlock::iterator &MBBI,
  ...
  while (Offset) {
    if (Offset > Chunk) {
      ...
      if (IsSub && !isRegLiveIn(MBB, WDC65816::D0))
        Reg = WDC65816::D0;
      else
        Reg = findDeadCallerSavedReg(MBB, MBBI, TRI);

      if (Reg) {
        unsigned Opc = WDC65816::MOV32ri;
        BuildMI(MBB, MBBI, DL, TII.get(Opc), Reg).addImm(Offset);
        Opc = IsSub ? WDC65816::SUB32ar : WDC65816::ADD32ar;
...
int WDC65816FrameLowering::mergeSPUpdates(MachineBasicBlock &MBB,
                                      MachineBasicBlock::iterator &MBBI,
  ...
  if (Opc == WDC65816::ADD32ai && PI->getOperand(0).getReg() == StackPtr) {
    assert(PI->getOperand(1).getReg() == StackPtr);
    Offset += PI->getOperand(2).getImm();
    MBB.erase(PI);
    if (!MergeWithPrevious)
      MBBI = NI;
  } else if (Opc == WDC65816::SUB32ai && PI->getOperand(0).getReg() == StackPtr) {
    assert(PI->getOperand(1).getReg() == StackPtr);
    Offset -= PI->getOperand(2).getImm();
    MBB.erase(PI);
    if (!MergeWithPrevious)
...
MachineInstrBuilder WDC65816FrameLowering::BuildStackAdjustment(
  ...
  unsigned Opc = IsSub ? WDC65816::SUB32ai : WDC65816::ADD32ai;
...
void WDC65816FrameLowering::BuildCFI(MachineBasicBlock &MBB,
                                 MachineBasicBlock::iterator MBBI,
                                 const DebugLoc &DL,
...
void WDC65816FrameLowering::emitPrologueCalleeSavedFrameMoves(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MBBI,
...
void WDC65816FrameLowering::emitPrologue(MachineFunction &MF,
  ...
  assert(&STI == &MF.getSubtarget<WDC65816Subtarget>() &&
  ...
  if (HasFP) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(WDC65816::LINK16))
        .addReg(WDC65816::WA6, RegState::Kill)
        .addImm(-NumBytes)
        .setMIFlag(MachineInstr::FrameSetup);
  ...
  while (MBBI != MBB.end() && MBBI->getFlag(MachineInstr::FrameSetup) &&
         MBBI->getOpcode() == WDC65816::PUSH32r) {
    PushedRegs = true;
    ++MBBI;
  ...
  if (TRI->hasBasePointer(MF)) {
    ...
    BuildMI(MBB, MBBI, DL, TII.get(WDC65816::MOV32aa), BasePtr)
        .addReg(SPOrEstablisher)
        .setMIFlag(MachineInstr::FrameSetup);
    if (MMFI->getRestoreBasePointer()) {
      // Stash value of base pointer.  Saving SP instead of FP shortens
      // dependence chain. Used by SjLj EH.
      unsigned Opm = WDC65816::MOV32ja;
      WDC65816::addRegIndirectWithDisp(BuildMI(MBB, MBBI, DL, TII.get(Opm)),
                                   FramePtr, true,
  ...
  // WDC65816 Interrupt handling function cannot assume anything about the
...
static bool isTailCallOpcode(unsigned Opc) {
  return Opc == WDC65816::TCRETURNj || Opc == WDC65816::TCRETURNq;
}
...
void WDC65816FrameLowering::emitEpilogue(MachineFunction &MF,
  ...
  while (MBBI != MBB.begin()) {
    ...
    if ((Opc != WDC65816::POP32r || !PI->getFlag(MachineInstr::FrameDestroy)) &&
        Opc != WDC65816::DBG_VALUE && !PI->isTerminator())
      break;
  ...
  if ((TRI->hasStackRealignment(MF) || MFI.hasVarSizedObjects())) {
    ...
    if (LEAAmount != 0) {
      unsigned Opc = WDC65816::LEA32p;
      WDC65816::addRegIndirectWithDisp(
          BuildMI(MBB, MBBI, DL, TII.get(Opc), StackPtr), FramePtr, false,
          LEAAmount);
      --MBBI;
    } else {
      BuildMI(MBB, MBBI, DL, TII.get(WDC65816::UNLK))
...
void WDC65816FrameLowering::determineCalleeSaves(MachineFunction &MF,
                                             BitVector &SavedRegs,
  ...
  int64_t TailCallReturnAddrDelta = WDC65816FI->getTCReturnAddrDelta();
...
bool WDC65816FrameLowering::assignCalleeSavedSpillSlots(
    MachineFunction &MF, const TargetRegisterInfo *TRI,
    std::vector<CalleeSavedInfo> &CSI) const {
  MachineFrameInfo &MFI = MF.getFrameInfo();
  WDC65816MachineFunctionInfo *WDC65816FI = MF.getInfo<WDC65816MachineFunctionInfo>();

  int SpillSlotOffset = getOffsetOfLocalArea() + WDC65816FI->getTCReturnAddrDelta();
...
bool WDC65816FrameLowering::spillCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
    ArrayRef<CalleeSavedInfo> CSI, const TargetRegisterInfo *TRI) const {
  auto &MRI = *static_cast<const WDC65816RegisterInfo *>(TRI);
  ...
  auto I =
      WDC65816::addFrameReference(BuildMI(MBB, MI, DL, TII.get(WDC65816::MOVM32pm)), FI)
          .addImm(Mask)
          .setMIFlag(MachineInstr::FrameSetup);
  ...
  for (const auto &Info : CSI) {
    ...
    WDC65816::addMemOperand(I, Info.getFrameIdx(), 0);
  }

  return true;
}

bool WDC65816FrameLowering::restoreCalleeSavedRegisters(
    MachineBasicBlock &MBB, MachineBasicBlock::iterator MI,
  ...
  auto I = WDC65816::addFrameReference(
               BuildMI(MBB, MI, DL, TII.get(WDC65816::MOVM32mp)).addImm(Mask), FI)
               .setMIFlag(MachineInstr::FrameDestroy);
