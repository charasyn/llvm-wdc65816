//===-- WDC65816CollapseMOVEMPass.cpp - Expand MOVEM pass -----------*- C++ -*-===//
//
...
#define PASS_NAME "WDC65816 MOVEM collapser pass"
...
class WDC65816CollapseMOVEM : public MachineFunctionPass {
public:
  static char ID;
  ...
  WDC65816CollapseMOVEM() : MachineFunctionPass(ID) {}
  ...
  void Finish(MachineBasicBlock &MBB, MOVEMState &State) {
    ...
    // Add a unified one
    if (State.isLoad()) {
      BuildMI(MBB, End, DL, TII->get(WDC65816::MOVM32mp))
          .addImm(State.getMask())
          .addImm(State.getFinalOffset())
          .addReg(State.getBase());
    } else {
      BuildMI(MBB, End, DL, TII->get(WDC65816::MOVM32pm))
          .addImm(State.getFinalOffset())
          .addReg(State.getBase())
          .addImm(State.getMask());
    }
...
char WDC65816CollapseMOVEM::ID = 0;
} // anonymous namespace.

INITIALIZE_PASS(WDC65816CollapseMOVEM, DEBUG_TYPE, PASS_NAME, false, false)

/// Returns an instance of the pseudo instruction expansion pass.
FunctionPass *llvm::createWDC65816CollapseMOVEMPass() {
  return new WDC65816CollapseMOVEM();
}
